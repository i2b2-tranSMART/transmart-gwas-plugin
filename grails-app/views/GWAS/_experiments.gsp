	...
	<div class="${(ti % 2) == 0 ? 'result-trial-odd' : 'result-trial-even'}" id="TrialDet_${experimentresult.key.id}_anchor">
		...
		<g:ifPlugin name='folder-management'>
			<span class="result-analysis-label">
				<g:set var='ts' value="${System.currentTimeMillis()}"/>
				<a id="toggleFile_${experimentresult.key.id}" href="#"
				   onclick="javascript:toggleFileDiv('${experimentresult.key.id}', '${createLink(controller: 'fmFolder', action: 'getFolderFiles',params: [id: experimentresult.key.id, trialNumber: experimentresult.key.id, unqKey: ts])}');">
					<img alt="expand/collapse" id="imgFileExpand_${experimentresult.key.id}"
					     src="${resource(file: '/images/down_arrow_small2.png')}" style="display: inline;"/>
					Files
				</a>
			</span>
		...
		<span class="result-analysis-label">
			...
			<a id="toggleDetail_${experimentresult.key.id}" href="#"
			   onclick="javascript:toggleDetailDiv('${experimentresult.key.id}', '${createLink(controller: 'GWAS', action: 'getTrialAnalysis',params: [id: experimentresult.key.id, trialNumber: experimentresult.key.id, unqKey: ts])}');">
