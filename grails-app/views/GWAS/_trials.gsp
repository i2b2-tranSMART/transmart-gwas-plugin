	...
	<div class="${(ti % 2) == 0 ? 'result-trial-odd' : 'result-trial-even'}" id="TrialDet_${trialresult.key.id}_anchor">
		...
		<span class="result-analysis-label">
			...
			<a id="toggleDetail_${experimentresult.key.id}" href="#"
			   onclick="javascript:toggleDetailDiv('${trialresult.key.trialNumber}', '${createLink(controller: 'GWAS', action: 'getTrialAnalysis',params: [id: trialresult.key.id, trialNumber: trialresult.key.trialNumber, unqKey: ts])}');">
				<img alt="expand/collapse" id="imgExpand_${trialresult.key.trialNumber}"
				     src="${resource('/images/down_arrow_small2.png')}" style="display: inline;"/>
				Analyses
			</a>
		</span>
