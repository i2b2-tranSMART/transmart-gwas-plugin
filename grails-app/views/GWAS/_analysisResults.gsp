...
<div id="analysis_results_table_${analysisId}_length_wrapper" class="dataTables_length">
	<label>Show
	<g:select name='resultCounter' size='1' from="${['10': '10', '25': '25', '50': '50', '100': '100']}"
	          optionKey="${{ it.key }}" optionValue="${{ it.value }}" id="analysis_results_table_${analysisId}_length"
	          onchange="loadAnalysisResultsGrid(${analysisId}, {max: jQuery(this).val()})" value="${max}"/> entries
	</label>
</div>
<div class="dataTables_filter">
	<label>P-value cutoff:
		<input type="text" style="width: 50px" id="analysis_results_table_${analysisId}_cutoff" onclick="removepvalue(${analysisId})">
	</label>
	<label>Search:
		<input value="${search}" type="text" style="width: 50px" id="analysis_results_table_${analysisId}_search">
		<span class="linkbutton" onclick="loadAnalysisResultsGrid(${analysisId}, {
			cutoff: jQuery('#analysis_results_table_${analysisId}_cutoff').val(),
			search: jQuery('#analysis_results_table_${analysisId}_search').val()
		})">OK</span>
	</label>
</div>
...
<div class="dataTables_info" id="analysis_results_table_${analysisId}_info">
	...
	<g:else>No entries to display</g:else>
</div>

<div class="dataTables_paginate paging_two_button" id="analysis_results_table_${analysisId}_paginate">
	<g:if test="${offset > 0}">
	<a class="paginate_enabled_previous" tabindex="0" role="button" id="analysis_results_table_${analysisId}_previous"
	   onclick="loadAnalysisResultsGrid(${analysisId}, {offset: ${offset-max}})">Previous</a>
	</g:if>
	<g:else>
	...
	<a class="paginate_enabled_next" tabindex="0" role="button" id="analysis_results_table_${analysisId}_next"
	   onclick="loadAnalysisResultsGrid(${analysisId}, {offset: ${offset+max}})">Next</a>
	</g:if>
	<g:else>
