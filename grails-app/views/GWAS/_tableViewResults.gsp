...
<div id="table_results_length_div" class="dataTables_length">
	<label>Show
	<g:select name='resultCounter' size='1' from="${['100': '100', '150': '150', '200': '200', '500': '500']}"
	          optionKey="${{ it.key }}" optionValue="${{ it.value }}" id="table_results_length"
	          onchange="loadTableResultsGrid({max: jQuery(this).val()})" value="${max}"/> ${type} results
	</label>
</div>
...
<div class="dataTables_filter">
	...
	<label>Search:
		...
		<span class="linkbutton" onclick="loadTableResultsGrid({
			cutoff: jQuery('#table_results_cutoff').val(),
			search: jQuery('#table_results_search').val()
		})">OK</span>
	</label>
</div>
</g:if>
...
<div class="dataTables_paginate paging_two_button">
	<g:if test="${offset > 0}">
	<a class="paginate_enabled_previous" tabindex="0" role="button" id="table_results_previous"
	   aria-controls="results_table" onclick="loadTableResultsGrid({offset: ${offset-max}})">Previous</a>
	</g:if>
	<g:else>
	<a class="paginate_disabled_previous" tabindex="0" role="button" id="table_results_previous"
	   aria-controls="results_table">Previous</a>
	</g:else>
	<g:if test="${offset + max < totalCount}">
	<a class="paginate_enabled_next" tabindex="0" role="button" id="table_results_next" aria-controls="results_table"
	   onclick="loadTableResultsGrid({offset: ${offset+max}})">Next</a>
	</g:if>
	<g:else>
